...
function createEventCards(eventsToRender = events) {
    ...
    eventsToRender.forEach(function(event) {
        ...
        const card     = document.createElement("div");    card.classList.add("dynamic-card");
        const heading  = document.createElement("h3");     heading.textContent = name;
        const details  = document.createElement("p");
        details.innerHTML = `<strong>Date:</strong> ${date}<br><strong>Category:</strong> ${category}<br><strong>Seats:</strong> <span class="seat-badge ${seats > 0 ? 'available' : 'booked'}">${seats}</span>`;
        const avail    = document.createElement("p");      avail.classList.add("availability-status");
        avail.textContent = event.checkAvailability();     avail.style.color = seats > 0 ? "#16a34a" : "#dc2626"; avail.style.fontWeight = "bold";
        const btnGroup = document.createElement("div");    btnGroup.classList.add("card-btn-group");
        const regBtn   = document.createElement("button"); regBtn.textContent = "Register"; regBtn.classList.add("btn-card-register");
        regBtn.onclick = function () { if (event.seats > 0) { event.seats--; displayEvents(); refreshCards(); alert(`Registered for ${name}!`); } else { alert(`${name} is fully booked.`); } };
        const cancelBtn = document.createElement("button"); cancelBtn.textContent = "Cancel"; cancelBtn.classList.add("btn-card-cancel");
        cancelBtn.onclick = function () { if (event.seats < maxSeats) { event.seats++; displayEvents(); refreshCards(); alert(`Cancelled for ${name}.`); } else { alert(`No registrations to cancel.`); } };
        btnGroup.appendChild(regBtn); btnGroup.appendChild(cancelBtn);
        card.appendChild(heading); card.appendChild(details); card.appendChild(avail); card.appendChild(btnGroup);
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelector("#registerSeatBtn").onclick  = function () { if (availableSeats > 0) { availableSeats--; document.querySelector("#seatCount").innerHTML = `Available Seats: ${availableSeats}`; alert("Registered!"); } };
...
window.onload = function () {
    ...
    updateClock();
    loadPreferences();
    console.group("🚀 Portal Initialised — Task 13 Debug Mode");
